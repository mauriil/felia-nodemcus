...
void callback(char *topic, byte *payload, unsigned int length)
{
  ...
  if(strcmp(topic, "/FELIA/POT_2") == 0)
  {
    ...
    Serial.print("Value: ");
    Serial.println(inStringESCMessage.toInt());
    FLAP.write(inStringESCMessage.toInt());
    inStringESCMessage = "";
  }
}
...
void loop()
{
  ...
  client.publish("/FELIA/temperature", String(bmp.readTemperature()).c_str(), true); // C*

  client.publish("/FELIA/pressure", String(bmp.readSealevelPressure()).c_str(), true); // Pa

  client.publish("/FELIA/altitude", String(bmp.readAltitude(seaLevelPressure_hPa * 100)).c_str(), true); // meters
}
